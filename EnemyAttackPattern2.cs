...
public class EnemyAttackPattern2 : MonoBehaviour
{
    projstart[] projstarts;
    ...
    private void Start()
    {
        projstarts = transform.GetComponentsInChildren<projstart>();
    }
    void Update()
    {
        LookatPlayer();
        ...
        if (Vector2.Distance(transform.position, player.position) < 5f)
        {
           if(timer > cooldown)
            {
                foreach (projstart proj in projstarts)
                {
                    proj.Shoot();
                }
                timer = 0;
